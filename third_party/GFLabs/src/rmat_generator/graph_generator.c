...
static int generate_4way_bernoulli(const CustomABC* custom_abc, mrg_state* st, int level, int nlevels) {
#else
...
#ifdef CUSTOM_INITIATOR
  uint32_t a = (uint32_t)(custom_abc->a * INITIATOR_DENOMINATOR);
  uint32_t b = (uint32_t)(custom_abc->b * INITIATOR_DENOMINATOR);
  uint32_t c = (uint32_t)(custom_abc->b * INITIATOR_DENOMINATOR);
...
static
#ifdef CUSTOM_INITIATOR
void make_one_edge(int64_t nverts, int level, int lgN, const CustomABC* custom_abc, mrg_state* st, packed_edge* result, uint64_t val0, uint64_t val1) {
#else
void make_one_edge(int64_t nverts, int level, int lgN, mrg_state* st, packed_edge* result, uint64_t val0, uint64_t val1) {
  ...
  while (nverts > 1) {
#ifdef CUSTOM_INITIATOR
    int square = generate_4way_bernoulli(custom_abc, st, level, lgN);
#else
...
#ifdef CUSTOM_INITIATOR
    make_one_edge(nverts, 0, logN, &custom_abc, &new_state, edges + (ei - start_edge), val0, val1);
#else
