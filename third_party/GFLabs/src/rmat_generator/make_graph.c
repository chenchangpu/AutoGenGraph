...
void make_graph(int log_numverts, int64_t M, uint64_t userseed1, uint64_t userseed2, 
#ifdef CUSTOM_INITIATOR
  /* in */ float initiator_a,
  /* in */ float initiator_b,
  /* in */ float initiator_c,
#endif 
  ...
  *result_ptr = edges;
  ...
  abc.b = initiator_b;
  abc.c = initiator_c;
  generate_kronecker_range(abc, seed, log_numverts, 0, M, edges);
#else
